...
const buildServices = () => {
    return serviceDefinitions.map((service, index) => {
        ...
        const spans = dependents.map((targetId, spanIndex) => {
            const status = spanIndex === 2 && (index + spanIndex) % 4 === 0 ? 'error' : 'ok';
            return {
...
const App = () => {
    return (_jsx("main", { style: { padding: '60px', background: '#e9ecf9', minHeight: '100vh' }, children: _jsx(TraceGraph, { services: services, edges: edges, defaultExpandedIds: services.map((svc) => svc.id) }) }));
};
export default App;
